...
export default {
  ...
  mounted: {
    query(app) {
      ...
      // layer.load(2);
      this.axios.post("/api/order/segDetailQryAll", app.qry).then((rtn) => {
        layer.close(layer.index);
        rtn.data.segDetails.map((x) => (x.isCheck = app.isCheckAll));
        app.segDetails = rtn.data.segDetails;

        if (app.qry.page.idx == 1) {
          this.axios.post("/api/order/segsTotalGet", app.qry).then((rtn) => {
...
function query(app) {
  ...
  // layer.load(2);
  axios.post("/api/order/segDetailQryAll", app.qry).then((rtn) => {
    layer.close(layer.index);
    rtn.data.segDetails.map((x) => (x.isCheck = app.isCheckAll));
    app.segDetails = rtn.data.segDetails;
