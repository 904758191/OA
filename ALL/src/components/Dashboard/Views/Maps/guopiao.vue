  ...
  <div>
    <div class="row">
      <form >
        ...
        <button class="btn btn-info btn-fill" @click="submit">查询</button>
      </form>
      <!-- 页面初始化时加载的表格 -->
      <div class="top">
        <ul>
          <li v-for="(air, index) in airCos" :key="index"> 
            <h3>
              {{ air
            ...
            <table border="1">
              ...
              <tr
                @click="input(r.route, air)"
                v-for="(r,index) in topData.filter((x) => x.airco === air)"
              :key="index">
                <td v-text="r.route"></td>
                <td v-text="r.segwait"></td>
              </tr>
            </table>
          </li>
        </ul>
...
export default {
  ...
  mounted() {
    this.axios.get("/order/tktWaitAll").then((rtn) => {
      this.topData = rtn;
      // alert(rtn.route);
      this.airCos = Array.from(new Set(this.topData.map(x => x.airco)))
    });
  },
  methods: {
    // 用于提交form表单
    submit() {
      // search是查询的意思
      // search(this);
      this.axios.post("/order/loadTrendGet").then(
        (res = {
          // this.segsFilted=res
        })
      );
    ...
    loadTrendGet(qry, app) {
      this.axios.post("/order/loadTrendGet", qry).then((rtn) => {
    ...
    tktCheckSubmit(app) {
      this.axios.post("/order/tktChkSave", app.gp).then((rtn) => {
        if (rtn.data.id > 0) {
    ...
    search(app) {
      ...
      //
      layer.load(2);
      this.axios.post("/order/segCntQry", app.qry).then((rtn) => {
        console.log(rtn);
        layer.closeAll();
