      ...
      <div class="infoOrder">
        ...
        <table class="mytable">
          ...
          <tr>
            ...
            <td
              v-text="order.data.pnrCode"
              @click="confirmTktIssue(order.data.pnrcode)"
            ></td>
            <td>
              <span v-if="order.locker != oper">{{ order.data.tripType }}</span>
              <select
                v-if="order.locker == oper"
                v-model="order.data.tripType"
                v-on:change="triptypechg()"
              >
                <option></option>
                <option>单程</option>
                <option>往返</option>
            ...
            <td>
              <input
                type="text"
                @change="orderRmkModi()"
                v-model="order.data.rmk"
                :readonly="order.locker != oper"
              />
            </td>
            <td>
              <span
                :style="{ color: order.data.UrgeTimes > 0 ? 'red' : 'black' }"
                >{{ order.data.UrgeTimes }}</span
              >
              <el-button
                type="warning"
                size="mini"
                @click="showReminder"
                style="padding: 3px 8px; margin-left: 5px;"
                >催单</el-button
              >
            </td>
            <td>
              ...
              <span class="sbtn" v-if="order.locker" @click="unlock()">
                解锁</span
              >
              <span class="sbtn" v-if="!order.locker" @click="lock()"
                >锁定</span
              >
            </td>
          </tr>
      ...
      <!-- <div class="infoRoute">
        ...
        <table class="mytable">
          ...
          <tr v-for="(route,index) of routes" :key="index">
                        <td>{{route.route}} <span v-if="route.isGaiQi==1" style="color:red;">改</span></td>
                        <td>{{route.timeDep}}</td>
                        <td>{{route.timeArr}}</td>
                        <td>{{route.flightNo}} <span style="color:red;" v-if="route.flightNo!=route.flightNoOrg">({{route.flightNoOrg}})</span></td>
...
export default {
  ...
  mounted() {
    ...
    this.orderno = location.search.qryobj().orderNo;
    
    function dataGet(qry, app) {
  // console.log(qry);
...
dataGet(location.search.qryobj(), this);
    setTimeout(orderLockerChk, 1000);
    setTimeout(this.getmatchs, 1000);
    // getQualityTestingInfo(this);
  function getQualityTestingInfo() {
  let orderNo = location.search.qryobj().orderNo;
  let qry = {
    page: {
      idx: 1,
      cnt: 100,
  ...
  methods: {
    ...
    selectAll: function(ctg) {
      ...
      if (ctg == "退票" || ctg == "改签") {
        ...
        let state = checkState(segs);
        if (this.isCheckAll) {
          this.segs.push({
            data: {
              airCoYinShou: this.segs[0].data.tktIssuePrice,
              psgrYinFu:
                parseFloat(this.segs[0].data.price) +
        ...
        let state = checkState(this.segExtra.filter((x) => x.data.ctg == ctg));
        if (this.isCheckAll) {
          this.segExtra.push({
            data: {
              ctg: ctg,
    ...
    checkOne: function(item, ctg) {
      ...
      if (ctg == "行李" || ctg == "餐食") {
        let state = checkState(this.checkedItems);
    ...
    chng: function(seg, type) {
      ...
      if (!seg.id || seg.data.pName == "") {
        ...
        if (seg.data.price != "") cal(seg, data, "price");
        if (seg.data.tax != "") cal(seg, data, "tax");
    ...
    showExtraDiv() {
      ...
      layer.open({
        ...
        moveOut: true,
          ...
          dataGet(location.search.qryobj(), app);
        },
      });
    },
    qcard(e) {
    ...
    yuantchngCheck: function(id) {
      //原票改签
      fnyuantchngCheck(id, this);
    },
    reoutchngCheck: function(id) {
      //重出改签
      fnreoutchngCheck(id, this);
    },
    RevokeChngCheck: function(id) {
      fnRevokeChngCheck(id, this);
    },
    ...
    checkpeopleshishou: function(pname) {
      fncheckpeopleshishou(pname, this);
    },
    checkpeopleshifu: function(pname) {
      fncheckpeopleshifu(pname, this);
    },
    addpeople: function() {
      var qry = location.search.qryobj();
      var people = {
    ...
    extraSave: function(extra) {
      ...
      if (extra.data.pName == "") {
        ...
        if (extra.data.rcv != "") cal(extra, unHandled, "rcv");
        if (extra.data.pay != "") cal(extra, unHandled, "pay");
        extra = unHandled;
      } else {
        if (
          extra.data.item == "" ||
...
<script>
function getQualityTestingInfo(app) {
  let orderNo = location.search.qryobj().orderNo;
  let qry = {
    page: {
      idx: 1,
...
function fncheckrefund(app) {
  ...
  if (checkRefundInfo != "") {
    layer.open({
      title: ["提示"],
      content:
        '<div style="color:#767676">确认提交退票<br/><b>' +
        checkRefundInfo +
        "</b>吗?</div>",
      btn: ["确认", "取消"],
      shadeClose: true,
...
function fnyuantchngCheck(id, app) {
  let tchng = app.chngsegs.filter((x) => x.id == id)[0];
  if (tchng.data.newflightNo == "") {
    layer.alert("请输入航班号!", {
      icon: 5,
  ...
  layer.open({
    title: ["提示"],
    content:
      '<div style="color:#767676">' + info + "确认提交原票改签数据?</div>",
    btn: ["确认", "取消"],
    shadeClose: true,
    //回调函数
    yes: function(index, layero) {
...
function fnTchngYuanSubmit(id, app) {
  let tchng = app.chngsegs.filter((x) => x.id == id)[0];
...
function fnreoutchngCheck(id, app) {
  let tchng = app.chngsegs.filter((x) => x.id == id)[0];
  if (tchng.data.newflightNo == "") {
    layer.alert("请输入航班号!", {
      icon: 5,
  ...
  layer.open({
    title: ["提示"],
    content:
      '<div style="color:#767676">' + info + "确认提交重出改签数据?</div>",
    btn: ["确认", "取消"],
    shadeClose: true,
    //回调函数
    yes: function(index, layero) {
...
function fnReoutTchngSubmit(id, app) {
  let tchng = app.chngsegs.filter((x) => x.id == id)[0];
...
function fnRevokeChngCheck(id, app) {
  let item = app.segs.filter((x) => x.id == id)[0];
  lock = false; //加个锁
  layer.open({
    title: ["提示"],
    content:
      '<div style="color:#767676">确认撤销：' +
      "<p/>行程：" +
      item.data.route +
      "<p/>姓名：" +
      item.data.pName +
...
function fnRevokeChngSubmit(id, app) {
...
function fncheckpeopleshishou(pname, app) {
  ...
  if (
    ...
    layer.open({
      title: ["提示"],
      content:
        '<div style="color:#767676">确认修改乘客信息：' +
        updateinfo +
        "<p/>实收：" +
        people.shishou +
        "<p/>实收账号：" +
...
function fnPeopleShiShouSubmit(pname, app) {
  let people = app.peoples.filter((x) => x.pName == pname)[0];
...
function fncheckpeopleshifu(pname, app) {
  let people = app.peoples.filter((x) => x.pName == pname)[0];
  ...
  layer.open({
    title: ["提示"],
    content:
      '<div style="color:#767676">确认添加改名实付信息：' +
      "<p/>实付：" +
      people.shifu +
      "<p/>实付账号：" +
      people.shifuacct +
...
function fnPeopleShiFuSubmit(pname, app) {
  let people = app.peoples.filter((x) => x.pName == pname)[0];
...
function fncheckaddpeople(app) {
  ...
  layer.open({
    title: ["提示"],
    content:
      '<div style="color:#767676">确认添加<b>[' + names + "]</b>吗?</div>",
    btn: ["确认", "取消"],
    shadeClose: true,
    //回调函数
    yes: function(index, layero) {
...
function fnAddPeoplesSubmit(app) {
  var addpeoples = app.newpeoples;
